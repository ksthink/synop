  ...
  return (
    <div className="relative">
      <button
        ...
        className="px-3 py-1.5 rounded text-sm text-neutral-500 hover:bg-neutral-100 hover:text-neutral-800 transition-colors"
      >
        버전
      </button>

      {open && (
        <div className="absolute right-0 top-9 z-30 w-64 bg-white border border-neutral-200 rounded-xl shadow-lg">
          <div className="flex justify-between items-center px-4 py-3 border-b border-neutral-100">
            <span className="text-sm font-medium text-neutral-700">버전 기록</span>
            <button
          ...
          <div className="max-h-64 overflow-y-auto">
            {loading ? (
              <p className="px-4 py-6 text-sm text-neutral-400 text-center">불러오는 중...</p>
            ) : versions.length === 0 ? (
              ...
              versions.map((v) => (
                ...
                >
                  <span className="text-sm text-neutral-600">{formatDate(v.createdAt)}</span>
                  <button
